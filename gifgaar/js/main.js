...
(function () {
	...
	a.init = function() {
		...
		var s = {
			...
			// example playlist
			playlist: ['track1.mp3'],
			playListLinks: ['https://soundcloud.com/trax-magazine/trax180-sleeparchive'],
			...
			trigger: 'chop',											// default visualization
	...
	a.bind = function() {
		...
		$('.fa-upload').on(click, function() { h.showModal('#modal-upload'); });
	...
	r.equal = function() {
		...
		$('body > svg').empty();
		var marginX = Math.floor(State.width/100);
		...
		var y = d3.scale.linear()
			.domain([0, 1])
			.range([0,State.height-marginY*5]);
